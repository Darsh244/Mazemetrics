...
void MazeSimulator::setPathFindingAlgorithm(const std::string &algorithm) {
  if (algorithm == currentPathFinderAlgorithm)
    return;
  if (!canSwitchPathFindingAlgorithm())
    return; // don't swap mid-search

  std::unique_ptr<PathFinder> previous = std::move(pathFinder);
  pathFinderMap[algorithm]();
  if (previous)
    pathFinder->transferStartEnd(*previous);

  currentPathFinderAlgorithm = algorithm;
}

bool MazeSimulator::canSwitchPathFindingAlgorithm() const {
  return state != STATE::FindingPath && state != STATE::PathFound;
}
...
void MazeSimulator::handleEvents(const sf::Event &event) {
  if (event.is<sf::Event::Closed>()) {
    ...
    if (mazeGenerator->generationDone()) {
      ...
      blockPos =
          blockPos / grid.getBlockSize(); // maps pixel position to row, col
...
void MazeSimulator::runSimulation() {
  ...
  if (state == STATE::PathFound) {
    ...
    if (pathFinder->isPathReconstructed())
      state = STATE::Finished;
  }
}

Grid &MazeSimulator::getGrid() { return grid; }

bool MazeSimulator::shouldCloseWindow() { return requestedWindowClose; }