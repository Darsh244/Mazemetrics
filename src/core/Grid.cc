...
void Grid::fill(const sf::Vector2u windowSize) {
  ...
  blockSize = floor(
      std::min(float(windowSize.x) / columns,
               float(windowSize.y) / rows)); // each block should be a square
  ...
  for (int row = 0; row < rows; ++row) {
    ...
    for (int col = 0; col < columns; ++col) {
      ...
      Block block(
          Type::WALL); // want border to be WALL so any even coord = Wall
      if (row % 2 == 1 && col % 2 == 1) {
...
void Grid::setBlockPosition(Position blockPos) {
  ...
  blockVertices[1].position =
      sf::Vector2f(c * blockSize + blockSize, r * blockSize);
  blockVertices[2].position =
      sf::Vector2f(c * blockSize + blockSize, r * blockSize + blockSize);
  blockVertices[3].position =
      sf::Vector2f(c * blockSize + blockSize, r * blockSize + blockSize);
  blockVertices[4].position =
      sf::Vector2f(c * blockSize, r * blockSize + blockSize);
...
bool Grid::outOfBounds(Position blockPos) {
  return blockPos.row < 0 || blockPos.row >= rows || blockPos.col < 0 ||
         blockPos.col >= columns;
}
