  ...
  for (int i = 0; i < batchSize; ++i) {
    if (!queue.empty() && !pathFound) {
      ...
      std::vector<Position> unexplored =
          getNonExploredNeighbours(current_block);
...
void BFS::reconstructPathStep(int batchSize) {
  for (int i = 0; i < batchSize; ++i) {
    if (currentPath != start.value()) {
      if (getBlockType(currentPath.value()) != Type::END)
        setBlockType(currentPath.value(), Type::PATH);
...
void BFS::addToQueue(Position blockPos) { queue.push(blockPos); }
