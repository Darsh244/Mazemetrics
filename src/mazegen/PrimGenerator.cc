...
PrimGenerator::PrimGenerator(Grid &g, unsigned int seed)
    : MazeGenerator(g, seed) {
  int rows = getRows();
...
void PrimGenerator::generateMazeStep(int batchSize) {
  for (int i = 0; i < batchSize; ++i) {
    if (!wallList.empty()) {
      std::pair<Wall, int> randomWallDetails =
          getRandomWallDetails(wallList, false);
      Wall randomWall = randomWallDetails.first;
      popWall(randomWallDetails.second, wallList);

      if (hasActive)
        setBlockType(activeBlock, Type::VISITED);
      if (onlyOneVisited(randomWall)) {
        ...
        Position between = {
            // since every pair of blocks have 2 units gap
...
Position PrimGenerator::getRandomPosition() {
  ...
  std::uniform_int_distribution<int> rowDist(
      0, ((rows - 1) / 2) - 1); // rows = 2 * r + 1; need upper bound as r-1
...
Position PrimGenerator::getUnvisited(Wall &wall) {
  if (getBlockType(wall.PosBlock1) == Type::UNVISITED)
    return wall.PosBlock1;
  else
    return wall.PosBlock2;
}

std::pair<PrimGenerator::Wall, int>
PrimGenerator::getRandomWallDetails(std::vector<Wall> &wallList, bool bias) {
  std::uniform_int_distribution<int> dist(0, wallList.size() - 1);
  int index = dist(generator);
  if (bias) {
    std::bernoulli_distribution chooseBack(1);
    if (chooseBack(generator))
      index = wallList.size() - 1; // generates longer corridors
  }
